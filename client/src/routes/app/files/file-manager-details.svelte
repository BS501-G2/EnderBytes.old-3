<script lang="ts">
  ...
  async function load(): Promise<Data> {
    ...
    const owner = await getUser([UserResolveType.UserId, file.ownerUserId]);
    const creator = await getUser([UserResolveType.UserId, file.creatorUserId]);
    ...
    const logs = await (async () => {
      ...
      for (const log of await listFileLogs(file.id)) {
        const user = await getUser([UserResolveType.UserId, log.actorUserId]);
