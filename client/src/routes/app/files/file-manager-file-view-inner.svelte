...
export interface FileViewInnerContext {
	pushTopRightContent: (view: Snippet) => () => void;
	pushBottomRightContent: (view: Snippet) => () => void;
	pushBottomLeftContent: (view: Snippet) => () => void;
}
</script>
...
<script lang="ts">
	...
	import { Banner, Button, Overlay, viewMode, ViewMode } from '@rizzzi/svelte-commons';
	import Icon, { type IconProps } from '$lib/ui/icon.svelte';
	import { scale } from 'svelte/transition';
	import ErrorCard from './file-views/error-card.svelte';
	import ImageViewer from './file-views/image-viewer.svelte';

	const { ...data }: FileViewData = $props();
	const { mime, viruses } = data;
...
<div class="file-content">
	{#if viruses.length}
		<ErrorCard
			title="File cannot be viewed"
			message="This file contains one or more viruses: {viruses.join(', ')}"
		/>
	{/if}
