  ...
  <div class="filter-actions">
    {#snippet checkbox(value: FileLogType)}
      <div class="filter-action">
        <input
          id="cb-{FileLogType[value]}"
          type="checkbox"
          name={FileLogType[value]}
          bind:group={filterActions}
          {value}
        />
        <label for="cb-{FileLogType[value]}">{FileLogType[value]}</label>
      </div>
    {/snippet}

    {@render checkbox(FileLogType.Create)}
    {@render checkbox(FileLogType.Modify)}
    {@render checkbox(FileLogType.Access)}
    {@render checkbox(FileLogType.Delete)}
    {@render checkbox(FileLogType.Restore)}
    {@render checkbox(FileLogType.Revert)}
    {@render checkbox(FileLogType.GrantAccess)}
    {@render checkbox(FileLogType.RevokeAccess)}
  </div>
{/snippet}
  ...
  {#snippet log2(log: FileLogResource)}
    {#snippet actions()}
      {#snippet button(name: string, icon: string, action: ActionButton)}
        <Button buttonClass={ButtonClass.Transparent} outline={false} hint={name} onClick={action}>
          <i class={icon}></i>
        </Button>
      {/snippet}
